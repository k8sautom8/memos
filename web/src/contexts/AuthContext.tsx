...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      ...
      // Silently handle unauthenticated errors (401) - this is expected when not logged in
      const isUnauthenticated = error instanceof Error && 
        (error.message.includes("Unauthenticated") || error.message.includes("401"));
