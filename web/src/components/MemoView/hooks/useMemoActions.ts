...
export const useMemoActions = (memo: Memo, isArchived: boolean) => {
  ...
  const pinMemo = async () => {
    ...
    await updateMemo({ update: { name: memo.name, pinned: true }, updateMask: ["pinned"] });
    toast.success(t("message.pinned-successfully"));
  };

  const unpinMemo = async () => {
    if (!memo.pinned) return;
    await updateMemo({ update: { name: memo.name, pinned: false }, updateMask: ["pinned"] });
    toast.success(t("message.unpinned-successfully"));
  };

  return { archiveMemo, pinMemo, unpinMemo };
};
