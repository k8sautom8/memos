...
interface MemoExportMenuProps {
  content: string;
  memoName?: string;
}

export default function MemoExportMenu({ content, memoName }: MemoExportMenuProps) {
  ...
  const handleExportMarkdown = () => {
    ...
    a.href = url;
    const filename = memoName ? `memo-${memoName}.md` : `memo-${new Date().toISOString().split("T")[0]}.md`;
    a.download = filename;
    a.click();
  ...
  const handleExportText = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    const filename = memoName ? `memo-${memoName}.txt` : `memo-${new Date().toISOString().split("T")[0]}.txt`;
    a.download = filename;
    a.click();
    URL.revokeObjectURL(url);
  };
...
${html}
    ...
    const a = document.createElement("a");
    a.href = url;
    const filename = memoName ? `memo-${memoName}.html` : `memo-${new Date().toISOString().split("T")[0]}.html`;
    a.download = filename;
    a.click();
    URL.revokeObjectURL(url);
  };
      ...
      <DropdownMenuTrigger asChild>
        <Button 
          ...
          className={cn(
            ...
            isColorfulTheme && !isDark && "hover:bg-emerald-500/12 hover:text-emerald-600 text-emerald-600/70"
          )}
          style={isDark ? {
        ...
        >
          <span style={isDark ? { color: "var(--card-foreground)", display: "inline-block" } : undefined}>
            <DownloadIcon className={cn("h-3.5 w-3.5 sm:h-4 sm:w-4", isColorfulTheme && !isDark && "text-emerald-600/70")} />
          </span>
        </Button>
      </DropdownMenuTrigger>
