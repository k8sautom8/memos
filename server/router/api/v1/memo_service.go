...
func (s *APIV1Service) CreateMemoComment(ctx context.Context, request *v1pb.CreateMemoCommentRequest) (*v1pb.Memo, error) {
	...
	// Find the root memo (the original post being commented on)
	// If relatedMemo is itself a comment (has a COMMENT relation), find its root memo
	rootMemo := relatedMemo
	memoRelationComment := store.MemoRelationComment
	memoRelations, err := s.Store.ListMemoRelations(ctx, &store.FindMemoRelation{
		MemoID: &relatedMemo.ID,
		Type:   &memoRelationComment,
	})
	if err == nil && len(memoRelations) > 0 {
		// relatedMemo has a COMMENT relation, so the related memo is the root
		rootMemoID := memoRelations[0].RelatedMemoID
		rootMemo, err = s.Store.GetMemo(ctx, &store.FindMemo{ID: &rootMemoID})
		if err != nil {
			// Fallback to relatedMemo if we can't find root
			rootMemo = relatedMemo
		}
	}
	...
	// Set ParentUID to the parent memo/comment (for nested comment support)
	parentUID := relatedMemo.UID
	err = s.Store.UpdateMemo(ctx, &store.UpdateMemo{
		ID:        memo.ID,
		ParentUID: &parentUID,
	})
	if err != nil {
		// If parent_uid column doesn't exist, that's okay - just log and continue
		// This allows the feature to work even if migration hasn't run yet
		slog.Warn("failed to set parent UID (column may not exist yet)", "error", err)
	}

	// Build the relation between the comment memo and the root memo (so it shows up in ListMemoComments)
	...
	if memoComment.Visibility != v1pb.Visibility_PRIVATE && creatorID != rootMemo.CreatorID {
